...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // REMOVED: JavaScript code that was adding duplicate admin link
  // Admin link is now handled by the HTML template ({% if role == 'admin' %})
  // This prevents duplicate "Admin Panel" links
});
  ...
  try {
    ...
    if (data.success) {
      ...
      data.pets.forEach(pet => {
        const option = document.createElement('option');
        option.value = pet.id;
        option.textContent = `${pet.name} (${pet.breed || 'Mixed Breed'})`;
        select.appendChild(option);
      });
    }
  } catch (error) {
      ...
      data.pets.forEach(pet => {
        ...
        petsHTML += `
          <div class="pet-card">
            ...
            <div class="pet-breed">${pet.breed || 'Mixed Breed'} • ${pet.age || 'Unknown'} years</div>
            <div class="pet-info">
...
function showEditPetImageModal(petId, petName) {
  ...
  for (let card of petCards) {
    ...
    if (nameEl && nameEl.textContent.trim() === petName) {
